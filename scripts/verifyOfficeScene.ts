...
const simulationSource = readFileSync(
  'src/features/command-room/OfficeSimulationModel.ts',
  'utf8',
)
...
assertIncludes(componentSource, 'onPointerDown={(event) => handleAgentPointerDown(event, station)}', 'physical agents support safe drag start')
assertIncludes(componentSource, 'onPointerMove={handleAgentPointerMove}', 'physical agents support drag movement')
assertIncludes(componentSource, 'onPointerUp={(event) => handleAgentPointerUp(event, station.agentId)}', 'physical agents return to simulation on drag release')
assertIncludes(componentSource, 'aria-pressed={station.agentId === selectedAgentId}', 'physical agents expose selected floor state')
assertIncludes(componentSource, 'const position = getWorldPoint(station.simulation.position)', 'physical agent x/y coordinates are converted from simulation position into locked world pixels')
assertIncludes(componentSource, 'const target = getWorldPoint(station.simulation.target)', 'physical agent target coordinates are converted from simulation target into locked world pixels')
assertIncludes(componentSource, "'--office-agent-x': `${position.x}px`", 'physical agent x coordinate uses fixed office-world pixels')
assertIncludes(componentSource, "'--office-agent-y': `${position.y}px`", 'physical agent y coordinate uses fixed office-world pixels')
assertIncludes(componentSource, "'--office-agent-heading': `${station.simulation.heading}deg`", 'physical agent heading uses simulation direction')
assertIncludes(componentSource, "'--office-agent-target-x': `${target.x}px`", 'physical agent target x coordinate uses fixed office-world pixels')
assertIncludes(componentSource, "'--office-agent-target-y': `${target.y}px`", 'physical agent target y coordinate uses fixed office-world pixels')
...
assertIncludes(componentSource, 'data-agent-heading={station.simulation.heading}', 'simulation heading metadata exposed in DOM')
...
assertIncludes(getBlock('.isometric-office'), '--office-world-max-height: clamp(620px, 58vw, 760px)', 'office scene root baseline gives the larger room vertical space')
assertIncludes(getBlock('.isometric-office'), 'place-items: center', 'office scene root centers the contained locked world')
assertIncludes(componentSource, 'className="office-world"', 'office scene renders a locked coordinate world wrapper')
assertIncludes(componentSource, 'data-office-world-width={OFFICE_WORLD.width}', 'office scene exposes fixed world width metadata')
assertIncludes(componentSource, 'data-office-world-height={OFFICE_WORLD.height}', 'office scene exposes fixed world height metadata')
assertIncludes(componentSource, 'officePercentToWorldPoint', 'route map converts simulation percentages into fixed office-world coordinates')
assertIncludes(componentSource, 'preserveAspectRatio="xMidYMid meet"', 'route SVG preserves the office world aspect ratio')
assertIncludes(componentSource, 'viewBox={`0 0 ${OFFICE_WORLD.width} ${OFFICE_WORLD.height}`}', 'route SVG uses the fixed office world viewBox')
assertIncludes(simulationSource, 'export const OFFICE_WORLD', 'office simulation model defines a fixed office-world coordinate system')
assertIncludes(simulationSource, 'width: 1536', 'office world width matches the 1536px background floorplan')
assertIncludes(simulationSource, 'height: 1024', 'office world height matches the 1024px background floorplan')
assertIncludes(simulationSource, 'officePercentToWorldPoint', 'office world exposes deterministic percent-to-world conversion')
assertIncludes(getBlock('.office-world-viewport'), 'aspect-ratio: 1536 / 1024', 'office world viewport preserves generated background aspect ratio')
assertIncludes(getBlock('.office-world'), 'width: var(--office-world-width, 1536px)', 'office world keeps a fixed internal pixel width')
assertIncludes(getBlock('.office-world'), 'height: var(--office-world-height, 1024px)', 'office world keeps a fixed internal pixel height')
assertIncludes(getBlock('.office-world'), 'scale(var(--office-world-scale, 1))', 'office world scales uniformly from the fixed internal canvas')
assertIncludes(getBlock('.office-world-viewport'), 'overflow: hidden', 'office world viewport clips all layers as one contained floorplan')
...
assertIncludes(getBlock('.office-floor'), 'inset: 0', 'office floor fills the locked coordinate world without crop offsets')
assertIncludes(getBlock('.office-floor'), "url('./assets/office-background.png')", 'room background lives on the office floor layer')
assertIncludes(getBlock('.office-floor'), 'contain no-repeat', 'room background scales as a contained floorplan, not cover-cropped')
...
assertIncludes(getBlock('.office-floor-agent'), 'pointer-events: auto', 'physical floor agents receive safe drag/select interaction')
assertIncludes(getBlock('.office-floor-agent'), 'touch-action: none', 'physical floor drag avoids unsafe page gestures')
assertIncludes(getBlock(".office-floor-agent[data-is-dragging='true']"), 'cursor: grabbing', 'dragging floor agent exposes grab-return affordance')
...
assertIncludes(getBlock(".office-floor-agent[data-agent-posture='working'] .office-agent-marker"), 'scaleY(0.86)', 'working posture lowers into a clear seated desk pose')
...
assertIncludes(getBlock('.office-agent-direction-arrow'), 'rotate(var(--office-agent-heading, 0deg))', 'walking direction arrow follows route heading')
assertNotIncludes(stylesheetSource, 'var(--office-agent-shift-x) + 16px', 'floor route animation must not add x drift away from model coordinates')
assertNotIncludes(stylesheetSource, 'var(--office-agent-shift-x) - 18px', 'floor route animation must not add reverse x drift away from model coordinates')
...
assert.equal(restingStation?.activity, 'resting', 'completed spec stays seated at the board/spec station')
